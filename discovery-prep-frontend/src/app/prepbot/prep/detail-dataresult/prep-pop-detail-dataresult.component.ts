...
export class PrepPopDetailDataresultComponent extends AbstractComponent implements OnInit, OnDestroy {
    ...
    @Output()
    public snapshotDetailCloseEvent:EventEmitter<boolean> = new EventEmitter();

    @Output()
    public navigateToDataflow = new EventEmitter();

    @Input()
    public pageType: string  = 'NORMAL';

    @Input()
    public pageSsid: string;
    ...
    /*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
     | Constructor
     |-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
    constructor(protected datasnapshotservice: SnapshotService,
    ...
    /*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
     ...
     |-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
    public ngOnInit() {
        if(this.pageType == 'NORMAL' && isNullOrUndefined(this.pageSsid)) {
            let ssId: string = '';
     ...
     */
    public close(refresh?:boolean) {
        this.resetComponent();

        if(this.pageType == 'NORMAL') {
            this.router.navigate(['/management/prepbot/']).then();
        }else{
            this.snapshotDetailCloseEvent.emit(refresh);
        }
    } // end of method close
     ...
     * */
    public confirmDelete(event, dataresult) {
        // stop event bubbling
        event.stopPropagation();
        this.deleteDataSnapshot();
        //
        // // delete modal
    ...
    public navigateDataflow(dfId) {
        this.router.navigate(['/management/prepbot/dataflow', dfId]).then();
    } // function - navigateDataflow


    private resetComponent() {
        this.ruleList  = null;
        this.sSInformationList = null;
     ...
     * Delete snapshot
     */
    private deleteDataSnapshot() {
        this.datasnapshotservice.deleteDataSnapshot(this.ssId).then(() => {
