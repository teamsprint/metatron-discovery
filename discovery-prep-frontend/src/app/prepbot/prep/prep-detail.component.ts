...
export class PrepDetailComponent extends AbstractComponent {
  ...
  public getSourceType(dataset: PrDataset) {
    let result:any = '';
    const iType: string = this.prepCommonUtil.getImportType(dataset.importType);

    if ('FILE' === iType || 'URI' === iType) {
      const ext = this.prepCommonUtil.getFileNameAndExtension(dataset.filenameBeforeUpload)[1];
